...
"""Jenkins-agent agent module tests."""
...
def test_agent_relation_changed_service_running(
    ...
    container = harness.model.unit.get_container("jenkins-agent")
...
def test_agent_relation_departed_container_not_ready(
    ...
    """
    mock_stop_agent = unittest.mock.MagicMock(spec=service.Service.stop_agent)
    monkeypatch.setattr(service.Service, "stop_agent", mock_stop_agent)
...
def test_agent_relation_departed(
    ...
    assert: the unit falls into BlockedStatus.
    """
    monkeypatch.setattr(service.Service, "stop_agent", lambda *_args, **_kwargs: None)
    mock_event = unittest.mock.MagicMock(spec=ops.RelationDepartedEvent)
    harness.set_can_connect("jenkins-agent", True)
    harness.begin()
