...
import charm_state
import service
from charm_state import AGENT_RELATION, DEFAULT_AGENT_USER, Credentials, State
...
def test_install_renders_custom_user_and_workdir(
    ...
    harness.update_config({"agent_user": DEFAULT_AGENT_USER, "jenkins_home": "/srv/jenkins"})
    harness.begin_with_initial_hooks()
    unit_text = host.unit_path.read_text()
...
def test_install_creates_user_and_home(
    ...
    harness.update_config({"agent_user": DEFAULT_AGENT_USER, "jenkins_home": str(home)})
    harness.begin_with_initial_hooks()

    assert home.exists()
    jenkins_uid = pwd.getpwnam(DEFAULT_AGENT_USER).pw_uid
    jenkins_gid = pwd.getpwnam(DEFAULT_AGENT_USER).pw_gid
...
def test_migrate_runtime_directories_skips_usable_entries(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    """
    Arrange: one runtime directory is usable and another needs migration.
    Act: request automatic runtime migration.
    Assert: only the unsafe runtime directory is traversed.
    """
    home = tmp_path / "jenkins-home"
    home.mkdir()
    remoting = home / "remoting"
