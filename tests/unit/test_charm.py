...
def test_upgrade_automatic_migration_rejects_symlinked_home(
    harness_with_agent_relation: ops.testing.Harness,
    monkeypatch: pytest.MonkeyPatch,
    service_mocks: SimpleNamespace,
    tmp_path: Path,
    ...
    Act: emit the upgrade hook.
    Assert: automatic migration blocks before touching the redirected tree.
    """
    real_home = tmp_path / "real-home"
    real_home.mkdir()
    home_link = tmp_path / "jenkins-home"
    home_link.symlink_to(real_home, target_is_directory=True)
    harness_with_agent_relation.update_config({"jenkins_home": str(home_link)})
    monkeypatch.setattr(
        service.JenkinsAgentService,
    ...
    harness_with_agent_relation.charm.on.upgrade_charm.emit()

    assert harness_with_agent_relation.charm.unit.status.name == ops.BlockedStatus.name
    assert "symbolic link" in harness_with_agent_relation.charm.unit.status.message
...
def test_migrate_runtime_directory_action_defaults_to_configured_home(
    ...
    assert output.results["user"] == charm_state.DEFAULT_AGENT_USER
