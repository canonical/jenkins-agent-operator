...
When `jenkins_home` is explicitly configured, the agent publishes it as the relation's
`remote_fs` metadata. The Jenkins controller consumes this value as the node's remote root
directory. With the default empty `jenkins_home` configuration, the agent uses
`/var/lib/jenkins` locally but publishes no `remote_fs` opinion, preserving any controller-side
